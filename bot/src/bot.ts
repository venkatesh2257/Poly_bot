...
function categorizeLoss(trade: Trade): string {
  if (trade.timeInWindow < 90) return "ARB_TOO_EARLY";
  if (trade.timeInWindow >= 90 && trade.timeInWindow <= 150) return "ARB_SWEET_SPOT";
...
async function onTick(price: number) {
  ...
  if (timeInWindow < 30 || timeInWindow > 120) return;
  ...
  if (!isOvernight) {
    ...
    if (tickCount % 300 === 1) console.log(`[skip] Active hours (UTC ${hourUTC}), overnight-only mode`);
    return;
  }
